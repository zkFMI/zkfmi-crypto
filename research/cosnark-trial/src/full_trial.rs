...
fn flip(hex: &mut String) {
    let old = hex.as_bytes()[0];
    hex.replace_range(..1, if old == b'0' { "1" } else { "0" });
}

pub fn verify_file(proof_path: &Path, statement_path: &Path) -> Result<(), String> {
    if fs::metadata(proof_path)
        .map_err(|_| "proof metadata")?
        .len()
        > 512 * 1024 * 1024
    {
        return Err("proof size limit".into());
    }
    let proof: Proof =
        serde_json::from_slice(&fs::read(proof_path).map_err(|_| "public proof read")?)
            .map_err(|_| "public proof syntax")?;
    let statement: Statement =
        serde_json::from_slice(&fs::read(statement_path).map_err(|_| "public statement read")?)
            .map_err(|_| "public statement syntax")?;
    verifier::verify(&proof, &statement)
}

pub fn execute(engine: &Path, output: &Path, manifest_path: &Path) -> Result<(), String> {
    let contract_bytes = fs::read("contract.json").map_err(|_| "contract missing")?;
    let manifest_bytes = fs::read(manifest_path).map_err(|_| "experiment manifest missing")?;
    ...
    let result = (|| -> Result<(), String> {
        ...
        standalone = Command::new(std::env::current_exe().map_err(|_| "verifier executable")?)
            ...
            .arg(&statement_path)
            .status()
            .map_err(|_| "standalone verifier launch")?
            .success();
        if !standalone {
            return Err("standalone public-only verifier rejected honest proof".into());
        }
        macro_rules! mutation {
            ($name:expr,$change:expr) => {{
                let mut changed = proof.clone();
                $change(&mut changed);
                negatives.insert(
                    $name.to_string(),
                    verifier::verify(&changed, &statement).is_err(),
                );
            }};
        }
        let mut changed = proof.clone();
        let mut another = statement.clone();
        another.deployment_id.push_str("-different-public-instance");
        changed.statement = another.clone();
        negatives.insert(
            "changed public statement".into(),
            verifier::verify(&changed, &another).is_err(),
        );
        mutation!("changed proof commitment root", |p: &mut Proof| flip(
            &mut p.roots[0].roots[0]
        ));
        mutation!("changed codeword opening", |p: &mut Proof| flip(
            &mut p.witness_opening.queries[0][0].original[0].value
        ));
        mutation!("changed final evaluation claim", |p: &mut Proof| {
            let mut y = unpack(&p.endpoint_claims, 4).unwrap();
            y[0] += F::from(1u64);
            p.endpoint_claims = pack(&y);
        });
        mutation!("changed deployment mode or identifier", |p: &mut Proof| p
            .statement
            .deployment_mode =
            "classical".into());
        ...
        let rejected = native
            .run(
                mpc_program::endpoints(&vec![F::from(2u64); 7]),
                test_view.view(),
                false,
                false,
                true,
            ...
            .is_err();
        let correct_marker = native
            .records
            .last()
            .and_then(|r| r.rejection_reason.as_deref())
            == Some("challenge view mismatch");
        negatives.insert(
            "inconsistent per-party challenge view".into(),
            rejected && correct_marker,
        );
        let rejected = native
            .run(
        ...
        let correct_marker = native
            ...
            .last()
            .and_then(|r| r.rejection_reason.as_deref())
            == Some("invalid financial witness");
        negatives.insert("invalid witness".into(), rejected && correct_marker);
        if negatives.len() != 7 || negatives.values().any(|pass| !*pass) {
            return Err("declared negative gate failed".into());
        }
        let guards = native.reject_replayed_queries(proof.witness_opening.final_rows.clone())?;
        let all = guards.len() == 7 && guards.iter().all(|v| *v);
        owner_query_guards = Some(guards);
        if !all {
            return Err("owner-local one-shot query guard failed".into());
        }
        Ok(())
    })();
    ...
    for path in [
        ...
        inputs.insert(
            path.into(),
            json!(hash(
                &fs::read(path).map_err(|_| "source identity input missing")?
            )),
        );
    }
    ...
    let receipt = json!({
        ...
        "owner_local_repeated_query_rejections":owner_query_guards,
    ...
    .map_err(|_| "public receipt output")?;
    let mut ledger = OpenOptions::new()
        .create(true)
        .append(true)
        .open("artifacts/ledger.jsonl")
        .map_err(|_| "append-only research ledger")?;
    writeln!(ledger, "{}", serde_json::to_string(&receipt).unwrap())
        .map_err(|_| "append result")?;
    ...
    if passed {
        ...
        Err("whole-candidate run blocked; see its public receipt".into())
    }
}
