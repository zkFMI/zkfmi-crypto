...
fn main() {
    ...
    if args.len()>1 {
        let result=match args[1].as_str() {
            ...
            "full" if args.len()==4 || args.len()==5 => zkfmi_cosnark_trial::full_trial::execute(Path::new(&args[2]),Path::new(&args[3]),Path::new(args.get(4).map(String::as_str).unwrap_or("experiment-002.json"))),
