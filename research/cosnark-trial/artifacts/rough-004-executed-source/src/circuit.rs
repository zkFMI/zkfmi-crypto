...
use crate::{algebra::eq_table, field::{encode, F}};
...
#[derive(Clone)]
pub struct Row { pub a: Linear, pub b: Linear, pub c: Linear }
...
impl Statement {
    pub fn smoke(deployment_id: String) -> Self {
        Self {
            ...
            deployment_mode: "pq-only".into(), deployment_id,
            circuit_sha512: fingerprint(), reserve: RESERVE,
            parties: PARTIES, max_corrupt: CORRUPT,
        }
    }
    pub fn validate(&self) -> Result<(), String> {
        if self != &Self::smoke(self.deployment_id.clone())
            || self.deployment_id.is_empty() || self.deployment_id.len() > 128 {
...
pub fn rows() -> Vec<Row> {
    let one = F::from(1u64);
    let mut rows = vec![
        Row { a: vec![(1,one)], b: vec![(2,one)], c: vec![(3,one)] },
        Row { a: vec![(3,one),(4,one),(0,-F::from(RESERVE))], b: vec![(0,one)], c: vec![] },
    ];
    for (value, start, bits) in [(1,5,8),(2,13,8),(4,21,16)] {
        let mut a = vec![(value,one)];
        a.extend((0..bits).map(|bit| (start+bit,-F::from(1u64 << bit))));
        rows.push(Row { a, b: vec![(0,one)], c: vec![] });
        for bit in 0..bits {
            rows.push(Row { a: vec![(start+bit,one)], b: vec![(start+bit,one),(0,-one)], c: vec![] });
        }
    }
    rows.resize_with(CONSTRAINTS, || Row { a: vec![], b: vec![], c: vec![] });
    rows
}
    ...
    for row in rows() {
        for linear in [&row.a, &row.b, &row.c] {
            ...
            for (column, value) in linear {
                hash.update((*column as u64).to_le_bytes()); hash.update(encode(*value));
            }
        }
    }
...
pub fn witness_opening_weights(point: &[F], batching: &[F]) -> Vec<F> {
    ...
    for (offset, length, coefficient) in [(R1,2,batching[0]),(R2,2,batching[1]),(R3,3,batching[2])] {
        for i in 0..length { weights[offset+i] += pad*beta[i]*coefficient; }
    }
