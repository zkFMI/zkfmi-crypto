...
use crate::{
    circuit::PARTIES, field::F, mpc_program, party::Request, pcs::NodeRoots, transcript::Hash,
};
...
impl Native {
    pub fn new(root: &Path, port: u16) -> Result<Self, String> {
        ...
        for party in 0..PARTIES {
            ...
            {
                return Err(
                    "refusing existing private persistence; use fresh isolated engine".into(),
                );
            }
        }
    ...
    pub fn reject_replayed_queries(
        ...
        for (party, worker) in self.workers.iter_mut().enumerate() {
            ...
            results.push(
                rejected
                    && status.code() == Some(1)
                    && log.contains("worker query budget was consumed or request is out of order"),
            );
        }
        Ok(results)
    }
