...
fn array(s:&mut String,name:&str,values:&[F]) {
    writeln!(s,"{name} = Array.create_from([{}], cint)",values.iter().map(ToString::to_string).collect::<Vec<_>>().join(",")).unwrap();
}
fn native_dot(s:&mut String,name:&str,offset:usize,weights:&[F],other:Option<(usize,F)>) {
    ...
    writeln!(s,"{name}_products = Array({}, sint)\n{name}_products.assign_vector(({expression}) * {name}_weights.get_vector())\n{name} = sum({name}_products[i] for i in range({}))",weights.len(),weights.len()).unwrap();
}
