...
fn restrict(values: &[F], r: F) -> Vec<F> {
    values.chunks_exact(2).map(|p| affine_pair(p[0], p[1], r)).collect()
}
...
fn rs_fold(codeword: &[F], r: F, normalize_even: bool) -> Vec<F> {
    ...
    (0..n / 2).map(|i| {
        ...
        let even_scale = if normalize_even { F::from(1u64) - r } else { F::from(1u64) };
...
fn decode_bounded(code: &[F], bound: usize) -> Option<Vec<F>> {
    ...
    if coefficients[bound..].iter().any(|v| *v != F::from(0u64)) { return None; }
...
fn combine(rows: &[Vec<F>]) -> Vec<F> {
    ...
    for (i, row) in rows.iter().enumerate() {
        ...
        for (v, item) in out.iter_mut().zip(row) { *v += weight * item; }
    }
    out
}
...
pub fn run() -> Observation {
    ...
    for party in 1..=ROWS {
        ...
        message.push((0..WIDTH).map(|j| {
            F::from((j + 2) as u64) + x * F::from((3 * j + 1) as u64)
                + x.square() * F::from((j + 5) as u64)
        }).collect::<Vec<_>>());
        mask.push((0..WIDTH).map(|j| {
            F::from((2 * j + 3) as u64) + x * F::from((j + 9) as u64)
    ...
    let weights: Vec<F> = [1u64, 3, 5, 15, 7, 21, 35, 105].into_iter().map(F::from).collect();
    let b = F::from(11u64);
    let r = F::from(13u64);
    let aggregate: Vec<Vec<F>> = message.iter().zip(&mask)
        .map(|(a, z)| a.iter().zip(z).map(|(a, z)| *a + b * z).collect()).collect();
    ...
    for row in &aggregate {
        ...
        literal_formula += usize::from(literal == row.chunks_exact(2).map(|p| p[0] + r * p[1]).collect::<Vec<_>>());
        for endpoint in [F::from(0u64), F::from(1u64)] {
            endpoints &= decode_bounded(&rs_fold(&encoded, endpoint, true), WIDTH / 2).unwrap() == restrict(row, endpoint);
        }
