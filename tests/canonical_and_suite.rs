...
#[test]
fn project_memory_is_well_formed_and_remote_only() {
    let project: toml::Value =
        toml::from_str(include_str!("../.codex/project-memory/project.toml")).unwrap();
    assert_eq!(
        project["verification"]["local_builds_forbidden"].as_bool(),
        Some(true)
    );
    assert_eq!(
        project["verification"]["allowed_remote_hosts"]
            .as_array()
            .unwrap()
            .len(),
        1
    ...
    for (input, fields) in [
        (
            include_str!("../.codex/project-memory/facts.jsonl"),
            &[
                "id",
                "recorded_at",
                "fact",
        ...
        (
            include_str!("../.codex/project-memory/decisions.jsonl"),
            &[
                "id",
                "recorded_at",
                "decision",
        ...
        (
            include_str!("../.codex/project-memory/worklog.jsonl"),
            &[
                "task_id",
                "recorded_at",
                "status",
