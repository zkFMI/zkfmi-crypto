...
use crate::{algebra::{dot,normalized_rs_fold,pack,unpack},circuit::{self,Statement}, field::{decode,domain,interpolate_row,F}, mpc_program::STATE_SIZE, pcs::{self,NodeRoots,PairOpening,PcsProof,CODE,PADDED}, transcript::{Merkle,Transcript},verifier::{self,ConstraintPrefix}};
use ark_ff::{BigInteger,Field,FftField,PrimeField,UniformRand};
...
pub enum Request {
    ...
    Commit,
    Fold { kind:usize, authorization:Authorization },
    Query { kind:usize, indices:Vec<usize>, final_rows:Vec<String> },
    Shutdown,
}
...
fn authorize(statement:&Statement,oracles:&[Merkle],party:usize,kind:usize,auth:&Authorization)->Result<(Transcript,F,F),String> {
    if &auth.prefix.statement!=statement || auth.prefix.roots.len()!=7
    ...
    let claim=if kind==2 {
        if auth.prior_mask_opening.is_some() { return Err("unexpected prior opening".into()); }
        ends[3]
    } else {
        let prior=auth.prior_mask_opening.as_ref().ok_or("missing prior mask opening")?;
        pcs::verify(&mut t,&auth.prefix.roots,2,&circuit::mask_opening_weights(&point),ends[3],prior)?;
    ...
    let yr=unpack(&auth.mask_claim,1)?[0]; t.absorb_fields(b"pcs-mask-claim",&[yr]);
    let b=t.field(b"pcs-aggregation"); let h=unpack(&auth.sumcheck,3)?;
    if h[0]+h[1]!=claim+b*yr { return Err("owner PCS sumcheck identity".into()); }
    t.absorb_fields(b"pcs-h",&h); let r=t.field(b"pcs-fold");
    if b==F::from(0u64) { return Err("zero masking challenge requires a fresh proof session".into()); }
    Ok((t,b,r))
}
...
fn load_own_oracles(root:&Path,party:usize)->Result<Vec<Merkle>,String> {
    ...
    for kind in 0..4 {
        ...
        if F::GENERATOR.pow([CODE as u64])==F::from(1u64) { return Err("query and secret-message domains overlap".into()); }
        let d=domain(CODE).map_err(str::to_owned)?.get_coset(F::GENERATOR).ok_or("invalid oracle coset")?;
        let mut coefficients=coefficients; coefficients.resize(CODE,F::from(0u64));
...
pub fn serve(root:&Path,party:usize)->Result<(),String> {
    ...
    loop {
        ...
        if line.len()>512*1024*1024 { return Err("worker request too large".into()); }
        let request:Request=serde_json::from_str(&line).map_err(|_|"worker request encoding")?;
        let answer=match request {
            Request::BindStatement{statement:given} => {
                if statement.is_some() { return Err("owner statement is already bound".into()); }
                given.validate()?; statement=Some(given); serde_json::json!({"party":party,"bound":true})
            },
            Request::Input{view,initial,invalid} => { write_input(root,party,&view,initial,invalid)?; serde_json::json!({"party":party,"view":view}) },
            Request::Commit => {
                if committed || statement.is_none() { return Err("owner commitment is one-shot and requires a bound statement".into()); }
                oracles=load_own_oracles(root,party)?; committed=true;
                serde_json::to_value(NodeRoots{roots:oracles.iter().map(Merkle::root).collect()}).unwrap()
            },
            Request::Fold{kind,authorization} => {
                if (kind!=0 && kind!=2)||oracles.len()!=4 || folded[kind/2] || (kind==0 && !queried[1]) { return Err("worker fold is out of order or its mask was consumed".into()); }
                let (cursor,b,r)=authorize(statement.as_ref().ok_or("owner statement missing")?,&oracles,party,kind,&authorization)?;
                ...
                folded[kind/2]=true; cursors[kind/2]=Some(cursor); own_final[kind/2]=Some(final_row.clone());
                serde_json::json!(final_row)
            },
            Request::Query{kind,indices,final_rows} => {
                if (kind!=0 && kind!=2)||oracles.len()!=4||indices.len()!=pcs::QUERIES||indices.iter().any(|i|*i>=CODE/2)
                    || !folded[kind/2] || queried[kind/2] { return Err("worker query budget was consumed or request is out of order".into()); }
                if final_rows.len()!=7 || own_final[kind/2].as_ref()!=Some(&final_rows[party]) { return Err("owner final-row view changed".into()); }
                let mut cursor=cursors[kind/2].take().ok_or("owner transcript cursor missing")?;
                pcs::absorb_final_rows(&mut cursor,&final_rows)?;
                let expected:Vec<usize>=(0..pcs::QUERIES).map(|_|cursor.query(CODE/2)).collect();
                if indices!=expected { return Err("owner Fiat-Shamir query view mismatch".into()); }
                queried[kind/2]=true;
