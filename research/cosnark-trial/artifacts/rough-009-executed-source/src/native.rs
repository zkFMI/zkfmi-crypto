...
impl Native {
    ...
    pub fn reject_replayed_queries(&mut self,final_rows:Vec<String>)->Result<Vec<bool>,String> {
        let request=Request::Query{kind:0,indices:vec![0;crate::pcs::QUERIES],final_rows};
        for worker in &mut self.workers { worker.send(&request)?; }
        let mut results=Vec::new();
        for (party,worker) in self.workers.iter_mut().enumerate() {
            let rejected=worker.receive::<serde_json::Value>().is_err();
            let status=worker.child.wait().map_err(|_|"wait one-shot guard result")?;
            let log=fs::read_to_string(self.root.join("cosnark-private-logs").join(format!("worker-{party}.log"))).map_err(|_|"owner one-shot guard evidence")?;
            results.push(rejected && status.code()==Some(1) && log.contains("worker query budget was consumed or request is out of order"));
        }
        Ok(results)
    }
    pub fn run(&mut self,source:String,view:Hash,initial:bool,invalid:bool,corrupt_views:bool)->Result<Vec<F>,String> {
        ...
        let compile=self.compiler.command().args(["-M","-F","64","-P",&mpc_program::prime(),&name]).output().map_err(|_|"launch official compiler")?;
        ...
        for party in 0..PARTIES {
            ...
            let child=Command::new(self.root.join("malicious-shamir-party.x"))
                ...
                .env("LD_LIBRARY_PATH",format!("{}:/opt/pqc-openssl/lib64",self.root.display()))
