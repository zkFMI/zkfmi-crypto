...
pub fn small_polynomial(values_at_integers: &[F], at: F) -> F {
    let nodes: Vec<F> = (0..values_at_integers.len())
...
pub fn normalized_rs_fold(codeword: &[F], r: F) -> Result<Vec<F>, String> {
    if codeword.len() < 2 {
        return Err("short codeword".into());
    }
    let d = domain(codeword.len())
        .map_err(str::to_owned)?
        .get_coset(F::GENERATOR)
        .ok_or("invalid code coset")?;
...
pub fn bounded_decode(codeword: &[F], bound: usize) -> Result<Vec<F>, String> {
    if bound > codeword.len() {
        return Err("invalid degree bound".into());
    }
    let d = domain(codeword.len())
        .map_err(str::to_owned)?
        .get_coset(F::GENERATOR.square())
        .ok_or("invalid folded coset")?;
...
pub fn unpack(encoded: &str, exact: usize) -> Result<Vec<F>, String> {
    ...
    bytes
        .chunks_exact(32)
        .map(|b| decode(b.try_into().unwrap()).ok_or("noncanonical field element".into()))
        .collect()
}
