...
fn values(n: usize) -> Vec<F> {
    (0..n).map(|i| F::from((i * i + 3 * i + 7) as u64)).collect()
}
...
fn coset_fold_matches_coefficient_restriction_including_endpoints() {
    ...
    let code = domain(64).unwrap().get_coset(F::GENERATOR).unwrap().fft(&padded);
    for challenge in [F::from(0u64), F::from(1u64), F::from(13u64)] {
        let folded = normalized_rs_fold(&code, challenge).unwrap();
        assert_eq!(bounded_decode(&folded, 8).unwrap(), fold_table(&coefficients, challenge));
...
fn message_and_coefficient_linear_functionals_match() {
    ...
    assert_eq!(dot(&weights, &message[..128]), dot(&pcs::coefficient_weights(&weights), &coefficients));
    let h0 = dot(&pcs::reply_message_weights(&weights, F::from(0u64)), &message);
    let h1 = dot(&pcs::reply_message_weights(&weights, F::from(1u64)), &message);
...
fn separable_mask_boolean_sum_has_declared_scaling() {
    ...
    let actual = (0..128).map(|index| {
        let point = (0..7).map(|bit| F::from(((index >> bit) & 1) as u64)).collect::<Vec<_>>();
        dot(&coefficients, &circuit::mask_opening_weights(&point))
    }).sum::<F>();
    let expected = coefficients[0] * F::from(128u64)
        + coefficients[1..circuit::MASK_LEN].iter().copied().sum::<F>() * F::from(64u64);
...
fn financial_fixture() -> Vec<F> {
    ...
    for (value, start, bits) in [(amount, 5, 8), (price, 13, 8), (remainder, 21, 16)] {
        for bit in 0..bits { z[start + bit] = F::from((value >> bit) & 1); }
    }
...
fn financial_relation_detects_inconsistent_notional() {
    ...
    let valid = |z: &[F]| circuit::rows().iter().all(|row| {
        linear(&row.a, z) * linear(&row.b, z) == linear(&row.c, z)
    });
    assert!(valid(&z));
    z[3] += F::from(1u64);
    assert!(!valid(&z));
...
fn collapsed_matrix_relation_binds_constant_and_padding_masks() {
    ...
    for (matrix, offset, length) in [(0, circuit::R1, 2), (1, circuit::R2, 2), (2, circuit::R3, 3)] {
        let mut table = circuit::rows().iter().map(|row| {
            linear([&row.a, &row.b, &row.c][matrix], &z)
        }).collect::<Vec<_>>();
        table.resize(128, F::from(0u64));
        table[64..64 + length].copy_from_slice(&z[offset..offset + length]);
        endpoint.push(mle(&table, &point));
    }
    assert_eq!(dot(&circuit::witness_opening_weights(&point, &batch), &z), dot(&endpoint, &batch[..3]) + batch[3]);
}
...
fn merkle_opening_binds_index_domain_value_and_salt() {
    ...
    assert_eq!(transcript::verify(&tree.root(), tree.domain(), 16, 5, &opening).unwrap(), values(16)[5]);
...
fn transcript_binds_statement_and_message_boundaries() {
    ...
    assert_eq!(F::GENERATOR.pow([pcs::CODE as u64]) == F::from(1u64), false);
}
