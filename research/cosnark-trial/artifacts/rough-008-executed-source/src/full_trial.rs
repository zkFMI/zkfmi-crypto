...
pub fn execute(engine:&Path,output:&Path,manifest_path:&Path)->Result<(),String> {
    ...
    let experiment_id=manifest["experiment_id"].as_str().ok_or("experiment identifier")?;
    ...
    let result=(||->Result<(),String> {
        ...
        if !all { return Err("owner-local one-shot query guard failed".into()); }
        Ok(())
    })();
    let mut inputs=serde_json::Map::new();
    inputs.insert(manifest_path.to_string_lossy().to_string(),json!(hash(&manifest_bytes)));
