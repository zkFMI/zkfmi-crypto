...
fn native_dot(s:&mut String,name:&str,offset:usize,weights:&[F],other:Option<(usize,F)>) {
    ...
    let terms=weights.iter().enumerate().filter(|(_,w)|**w!=F::from(0u64)).map(|(i,w)| {
        let value=if let Some((mask,b))=other { format!("(state[{}] + state[{}] * {b})",offset+i,mask+i) }
            else { format!("state[{}]",offset+i) };
        format!("{value} * {w}")
    }).collect::<Vec<_>>();
    let expression=if terms.is_empty(){"sint(0)".to_owned()}else{terms.join(" + ")};
