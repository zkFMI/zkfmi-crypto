...
use crate::{
    algebra::{bounded_decode, dot, fold_table, lagrange_at, small_polynomial, unpack},
    circuit::PARTIES,
    field::{domain, F},
    transcript::{self, Opening, Transcript},
};
...
pub fn oracle_domain(kind: usize, party: usize) -> Vec<u8> {
    // A persisted book moves from output (6) to next input (4) unchanged.
    let kind = if kind == 6 { 4 } else { kind };
...
pub fn coefficient_weights(message_weights: &[F]) -> Vec<F> {
    ...
    weights.resize(PADDED, F::from(0u64));
...
pub fn reply_message_weights(message_weights: &[F], at: F) -> Vec<F> {
    ...
    for weight in reduced {
        coefficient.push((F::from(1u64) - at) * weight);
        coefficient.push(at * weight);
    }
    domain(PADDED).unwrap().ifft(&coefficient)
}

pub fn absorb_final_rows(
    transcript: &mut Transcript,
    rows: &[String],
) -> Result<Vec<Vec<F>>, String> {
    if rows.len() != PARTIES {
        return Err("wrong final-row count".into());
    }
    rows.iter()
        .enumerate()
        .map(|(i, row)| {
...
pub fn verify(
    ...
    verify_roster(transcript, roots, kind, weights, claim, proof, 4)
}

/// Same PCS with an explicitly fixed integration roster, not variable user
/// supplied dimensions. Legacy entry point still accepts exactly four roots.
...
pub fn verify_roster(
    ...
    if ![4, 8].contains(&count) || !kind.is_multiple_of(2) || kind >= count {
        return Err("invalid oracle kind".into());
    }
    if roots.len() != PARTIES || roots.iter().any(|r| r.roots.len() != count) {
        return Err("root roster shape".into());
    }
    ...
    if h[0] + h[1] != claim + b * mask_claim {
        return Err("PCS sumcheck initial identity".into());
    }
    ...
    let decoded = rows
        .iter()
        .map(|row| bounded_decode(row, PADDED / 2))
    ...
    for query in &proof.queries {
        ...
        if query.len() != PARTIES {
            return Err("PCS query roster".into());
        }
        for (party, opening) in query.iter().enumerate() {
            let mut aggregate = [F::from(0u64); 2];
            for (side, value) in aggregate.iter_mut().enumerate() {
                let at = index + side * (CODE / 2);
                let original = transcript::verify(
                    &roots[party].roots[kind],
                    &oracle_domain(kind, party),
                    CODE,
                    at,
                    &opening.original[side],
                )?;
                let mask = transcript::verify(
                    &roots[party].roots[kind + 1],
                    &oracle_domain(kind + 1, party),
                    CODE,
                    at,
                    &opening.mask[side],
                )?;
                *value = original + b * mask;
            }
